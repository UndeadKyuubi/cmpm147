// sketch.js - Generates an infinite game of mine sweeper
...
let tileSafetyMap = new Map();
let revealedSafeTiles = new Map();

function countSurroundingBombs(i, j) {
  let bombCount = 0;
  ...
  for (let offset of offsets) {
    let neighborI = i + offset[0];
    let neighborJ = j + offset[1];
    if (tileSafetyMap.has(`${neighborI},${neighborJ}`)) {
      if (!tileSafetyMap.get(`${neighborI},${neighborJ}`)) {
        bombCount++;
      }
    }
  }
...
function p3_drawTile(i, j) {
  ...
  if (i === 0 && j === 0) {
    fill(240, 200);
    tileSafetyMap.set(`${i},${j}`, true);
  } else if (XXH.h32("tile:" + [i, j], worldSeed) % 4 == 0) {
    fill(240, 200);
    tileSafetyMap.set(`${i},${j}`, false);
  } else {
    fill(240, 200);
    tileSafetyMap.set(`${i},${j}`, true);
  }
  ...
  let n = clicks[[i, j]] | 0;
  if (n % 2 == 1 && tileSafetyMap.get(`${i},${j}`) === true) {
    fill(0, 0, 0, 32);
    ellipse(0, 0, 10, 5);
    translate(0, -10);
    fill(255, 255, 100, 128);
    ellipse(0, 0, 10, 10);
    revealedSafeTiles.set(`${i},${j}`, true);
  }
  else if (n % 2 == 1 && tileSafetyMap.get(`${i},${j}`) === false){
    fill(255, 0, 0);
    beginShape();
    vertex(-tw, 0);
    vertex(0, th);
...
function p3_drawSelectedTile(i, j) {
  ...
  noStroke();
  fill(0);
  if (revealedSafeTiles.get(`${i},${j}`) === true){
    text("bombs " + countSurroundingBombs(i,j), 0, 0);
  }
}
...
function setup() {
  ...
  createP("Arrow keys scroll. Clicking reveals tiles.").parent("canvas-container");
